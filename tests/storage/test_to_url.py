...
def test_to_url():
    ...
    # public bucket
    assert (
        ln_setup.core.upath.UPath("s3://lamindata/test-folder").to_url()
        == "https://lamindata.s3.amazonaws.com/test-folder"
    )
    # private bucket
    assert (
        ln_setup.core.upath.UPath("s3://lamindb-setup-private-bucket/test-folder")
    ...
    assert (
        ln_setup.core.upath.UPath("s3://lamin-eu-central-1/test-folder").to_url()
